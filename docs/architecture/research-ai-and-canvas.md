...
Вывод: на бэкенде держим один адаптер на пакете `openai` и переключаем `base_url` / `api_key` / `model`. Большинство провайдеров OpenAI-совместимы. Anthropic и собственный SDK Gemini подключаются отдельными адаптерами.
