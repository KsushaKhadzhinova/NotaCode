...
Архитектура определяется архитектурными характеристиками (Richards & Ford). Наибольшее влияние на структуру оказывают следующие драйверы:
...
| D2 | Много строгих нотаций с правилами (UML ×14, BPMN ×4, ERD, IDEF0/1X/3, DFD, Петри ×N) | ТЗ | Ядро языка отделяется от нотаций; нотация реализуется плагином (Open/Closed) |
...
| D6 | Требования лаб ИТиВП (Node.js-сервер, React, тесты, деплой, мониторинг) | методички | Node.js используется в роли edge/BFF; предметная логика реализуется на Python |
...
| Микросервисы (15+ мелких) | Максимальная независимость | Newman и Fowler («MonolithFirst») предупреждают: без зрелых границ это распределённый монолит. Высокая стоимость эксплуатации | Нет |
...
- Один сервис — один ограниченный контекст — своя схема БД. Доступ к данным другого сервиса осуществляется только через API или события (Newman, «database per service»).
...
| Контейнер | Контекст DDD | Владеет данными | Обоснование выделения |
|---|---|---|---|
...
| **Edge Gateway/BFF** (Node.js, Express) | Identity & Access, Notifications | PostgreSQL `identity` (Sequelize): users, credentials, refresh_tokens, settings, linked_accounts; MongoDB (Mongoose): notifications, activity | Паттерн BFF/API Gateway (Newman, Richardson): единая точка входа и контроля безопасности; нагрузка I/O-bound соответствует модели исполнения Node.js; закрывает лабы ч.2: Express (ЛР1), Sequelize (ЛР2), JWT (ЛР3), MongoDB (ЛР6), Socket.IO (ЛР7), Docker (ЛР8) |
...
Обоснование собственного SVG-рендера: API maxGraph (версия 0.x) изменяется между релизами; React Flow отображает узлы HTML-элементами, что усложняет строгие фигуры нотаций и SVG-экспорт; bpmn-js ограничен нотацией BPMN и требует водяного знака. Полный редактор draw.io подключается отдельной командой «Открыть в draw.io» (embed + postMessage).
